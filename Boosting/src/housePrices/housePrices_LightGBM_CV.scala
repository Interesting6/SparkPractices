...
object housePrices_LightGBM_CV {
...
//    Logger.getLogger("org.apache.eclipse.jetty.server").setLevel(Level.OFF)
    ...
    // 创建一个LightGBM回归器
    val gbm = new LightGBMRegressor().setLabelCol("label").setFeaturesCol("features")
    ...
    val gbmParamGrid = new ParamGridBuilder()
      ...
      .addGrid(gbm.objective, Array("regression")) //multiclassova
    ...
    //创建交叉验证的Pipeline，用LightGBM作为estimator，用RegressionEvaluator作为evaluator，用gbmParamGrid作为hyperparameters
    ...
    // 模型保存
    bestModel.write.overwrite().save("./saveModel/spark-lightgbm-regression-model-housePrices")

    // 模型读取
    val loadModel = PipelineModel.load ("./saveModel/spark-lightgbm-regression-model-housePrices")
