...
//      .setOutputCol("label")
    ...
    // 创建一个LightGBM分类器
    val gbm = new LightGBMClassifier().setLabelCol("label").setFeaturesCol("features")
...
//      .addGrid(gbm.learningRate, Array(0.1))
      .addGrid(gbm.objective, Array("multiclass")) //multiclassova
//      .addGrid(xgb.numRound,Array(100))
      .build()

    // 创建一个LightGBM的pipeline
    val pipeline = new Pipeline().setStages(indexers ++ encoders ++ Array(vectorAssembler,gbm))
    ...
    //创建交叉验证的Pipeline，用 LightGBM作为estimator，用MulticlassClassificationEvaluator作为evaluator，用gbmParamGrid作为hyperparameters
    ...
    // 模型保存
    bestModel.write.overwrite().save("./saveModel/spark-lightgbm-classifier-model-covType")

    // 模型读取
    val loadModel = PipelineModel.load ("./saveModel/spark-lightgbm-classifier-model-covType")
