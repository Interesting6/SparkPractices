...
//    Logger.getLogger("org.apache.eclipse.jetty.server").setLevel(Level.OFF)
    ...
    // 创建一个XGBoost分类器
    val xgb = new XGBoostClassifier().setLabelCol("label").setFeaturesCol("features").setMissing(0)
    ...
    val pipeline = new Pipeline().setStages(Array(labelIndexer) ++ indexers ++ encoders ++ Array(vectorAssembler,xgb))
    ...
    // 模型保存
    bestModel.write.overwrite().save("./saveModel/spark-xgboost-classifier-model-bank-normal")

    // 模型读取
    val loadModel = PipelineModel.load ("./saveModel/spark-xgboost-classifier-model-bank-normal")
