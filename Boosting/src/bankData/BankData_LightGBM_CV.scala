...
//    Logger.getLogger("org.apache.eclipse.jetty.server").setLevel(Level.OFF)
    ...
    // 创建一个LightGBM分类器,设置分类数为12
    val gbm = new LightGBMClassifier().setLabelCol("label").setFeaturesCol("features").setBoostFromAverage(false)

    // LightGBM paramater grid
    val gbmParamGrid = new ParamGridBuilder()
      //      .addGrid(gbm.maxDepth, Array(4,8))
      //      .addGrid(gbm.minSumHessianInLeaf, Array(0.001))
      //      .addGrid(gbm.numLeaves, Array(31))
      //      .addGrid(gbm.lambdaL1, Array(0.0))
      //      .addGrid(gbm.lambdaL2, Array(0.6))
      //      .addGrid(gbm.learningRate, Array(0.1))
      .addGrid(gbm.objective, Array("binary")) //multiclassova
      //      .addGrid(xgb.numRound,Array(100))
      .build()

    // 创建一个LightGBM的pipeline
    val pipeline = new Pipeline().setStages(Array(labelIndexer) ++ indexers ++ encoders ++ Array(vectorAssembler,gbm))
    ...
    //创建交叉验证的Pipeline，用 XGBoost作为estimator，用BinaryClassificationEvaluator作为evaluator，用xgbParamGrid作为hyperparameters
    val cv = new CrossValidator()
      .setEstimator(pipeline)
      .setEvaluator(evaluator)
      .setEstimatorParamMaps(gbmParamGrid)
      .setNumFolds(3)
      .setSeed(0)
    ...
    // 模型保存
    bestModel.write.overwrite().save("./saveModel/spark-lightgbm-classifier-model-bank-normal")

    // 模型读取
    val loadModel = PipelineModel.load ("./saveModel/spark-lightgbm-classifier-model-bank-normal")
