...
object BankData_DecisionTree_CV {
...
//    Logger.getLogger("org.apache.eclipse.jetty.server").setLevel(Level.OFF)
    ...
    val pipeline = new Pipeline().setStages(Array(labelIndexer) ++ indexers ++ encoders ++ Array(vectorAssembler,dt))
    ...
    //创建交叉验证的Pipeline，用 DecisionTree作为estimator，用BinaryClassificationEvaluator作为evaluator，用dtParamGrid作为hyperparameters
    ...
    // 模型保存
    bestModel.write.overwrite().save("./saveModel/spark-dt-classifier-model-bank")

    // 模型读取
    val loadModel = PipelineModel.load ("./saveModel/spark-dt-classifier-model-bank")
