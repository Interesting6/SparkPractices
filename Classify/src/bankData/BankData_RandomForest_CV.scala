...
object BankData_RandomForest_CV {
...
//      .addGrid(rf.minInfoGain, Array(0.1))
    ...
    val pipeline = new Pipeline().setStages(Array(labelIndexer) ++ indexers ++ encoders ++ Array(vectorAssembler,rf))
    ...
    //创建交叉验证的Pipeline，用 RandomForest作为estimator，用BinaryClassificationEvaluator作为evaluator，用rfParamGrid作为hyperparameters
    ...
    // 模型保存
    bestModel.write.overwrite().save("./saveModel/spark-rf-classifier-model-bank")

    // 模型读取
    val loadModel = PipelineModel.load ("./saveModel/spark-rf-classifier-model-bank")
