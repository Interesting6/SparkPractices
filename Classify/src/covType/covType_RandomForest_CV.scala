...
object covType_RandomForest_CV {
...
//      .setOutputCol("label")
    ...
    // RandomForest paramater grid
    val rfParamGrid = new ParamGridBuilder()
//      .addGrid(rf.numTrees, Array(20))
      .addGrid(rf.maxDepth, Array(4,8))
//      .addGrid(rf.minInstancesPerNode, Array(1))
//      .addGrid(rf.minInfoGain, Array(0.1))
      .addGrid(rf.impurity, Array("gini"))
      .build()

    // 创建一个RandomForest的pipeline
    val pipeline = new Pipeline().setStages(indexers ++ encoders ++ Array(vectorAssembler,rf))
    ...
    //创建交叉验证的Pipeline，用 RandomForest作为estimator，用MulticlassClassificationEvaluator作为evaluator，用rfParamGrid作为hyperparameters
    ...
    // 模型保存
    bestModel.write.overwrite().save("./saveModel/spark-rf-classifier-model-covType")

    // 模型读取
    val loadModel = PipelineModel.load ("./saveModel/spark-rf-classifier-model-covType")
