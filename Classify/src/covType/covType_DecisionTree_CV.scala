...
object covType_DecisionTree_CV {
...
//      .setOutputCol("label")
    ...
    // DecisionTree paramater grid
    val dtParamGrid = new ParamGridBuilder()
      .addGrid(dt.maxDepth, Array(4,5,6))
//      .addGrid(dt.minInstancesPerNode, Array(1))
//      .addGrid(dt.minInfoGain, Array(0.1))
//      .addGrid(dt.impurity, Array("gini"))
      .build()

    // 创建一个DecisionTree的pipeline
    val pipeline = new Pipeline().setStages(indexers ++ encoders ++ Array(vectorAssembler,dt))
    ...
    //创建交叉验证的Pipeline，用 DecisionTree作为estimator，用MulticlassClassificationEvaluator作为evaluator，用dtParamGrid作为hyperparameters
    ...
    // 划分数据集
    val Array(trainingData, testData) = dataset.randomSplit(Array(0.8, 0.2), seed=1234)
    ...
    // 模型保存
    bestModel.write.overwrite().save("./saveModel/spark-dt-classifier-model-covType")

    // 模型读取
    val loadModel = PipelineModel.load ("./saveModel/spark-dt-classifier-model-covType")
