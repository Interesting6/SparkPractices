...
object housePrices_GradientBoostedTree_CV {
...
//      .addGrid(gbt.minInfoGain, Array(0.1))
      .addGrid(gbt.impurity, Array("variance"))
      .build()

    // 创建一个DT的pipeline
    val pipeline = new Pipeline().setStages(indexers ++ encoders ++ Array(vectorAssembler,gbt))
    ...
    //创建交叉验证的Pipeline，用 GBT作为estimator，用RegressionEvaluator作为evaluator，用gbtParamGrid作为hyperparameters
    ...
    // 模型保存
    bestModel.write.overwrite().save("./saveModel/spark-gbt-regression-model-housePrices")

    // 模型读取
    val loadModel = PipelineModel.load ("./saveModel/spark-gbt-regression-model-housePrices")
